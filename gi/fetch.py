...
from .util import get_cache_dir, get_index_cache_path, get_template_cache_path, is_stale_cache


class GitIgnoreFetcher:
    """Handles fetching and caching .gitignore templates."""
    
    def __init__(self, base_url: str = "https://raw.githubusercontent.com/github/gitignore/HEAD"):
    ...
    def get_index(self, force: bool = False) -> Dict:
        ...
        try:
            ...
            for item in response.json():
                if item["type"] == "file" and item["name"].endswith(".gitignore"):
                    ...
                    for global_item in global_response.json():
                        if global_item["type"] == "file" and global_item["name"].endswith(".gitignore"):
                            templates.append({
            ...
            raise RuntimeError(f"Failed to fetch template index: {e}")
    ...
    def get_template(self, template_name: str, no_cache: bool = False) -> str:
        ...
        try:
            ...
            raise RuntimeError(f"Failed to fetch template '{template_name}': {e}")
    ...
    def list_templates(self) -> List[str]:
        ...
        try:
            index = self.get_index()
            return [template["name"][:-10] for template in index["templates"]]  # Remove .gitignore suffix
        except RuntimeError:
            return []
