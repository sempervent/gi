...
@app.command()
def main(
    templates: str = typer.Argument(..., help="Template names to combine (space or comma separated)"),
    ...
    # Write output file
    if not safe_write_file(output, combined_content, force=force):
        if output.exists():
    ...
    console.print(f"Combined {len(templates_content)} template(s): {', '.join(templates_content.keys())}")
...
def list_templates() -> None:
    ...
    try:
        ...
        if not templates:
            ...
            console.print("Try running with [bold]--update-index[/bold] to refresh the list.")
            return
...
def search(
    ...
    try:
        ...
        for template in sorted(matches):
            category = "Global/Editor" if template.startswith("Global/") else "Language/Framework"
...
def doctor() -> None:
    ...
    from .util import get_cache_dir, get_index_cache_path
    from .fetch import get_fetcher
    import json
    import time
    ...
    console.print(f"Cache exists: [green]{'Yes' if cache_dir.exists() else 'No'}[/green]")
    ...
    if index_path.exists():
        try:
            ...
            console.print(f"Index exists: [green]Yes[/green]")
            ...
            if is_stale_cache(index_path):
                console.print("Index status: [yellow]Stale (older than 24 hours)[/yellow]")
            else:
    ...
    if template_files:
        ...
        for template_file in sorted(template_files):
            ...
            mtime = time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(template_file.stat().st_mtime))
            console.print(f"  [cyan]{template_file.name}[/cyan] ({size} bytes, {mtime})")
    ...
    try:
        ...
        console.print(f"Test fetch (Python): [green]OK[/green] ({len(test_content)} bytes)")
    except Exception as e:
        console.print(f"GitHub connectivity: [red]Failed[/red]")
